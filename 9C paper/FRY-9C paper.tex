...
\def\BibTeX{{\rm B\
		kern-.05em{\sc i\kern-.025em b}\kern-.08em
...
\section{Comparison of data in FR Y-9C with other reports submitted to the Federal Reserve}
	...
	\begin{table}[htbp]
		...
		\begin{tabular}{|l|p{7cm}|}
			...
			\hline
			FR-2314 & Financial Statements of Foreign Subsidiaries of U.S. Banking Organizations \\
			\hline
		\end{tabular}
		\label{tab:fed_forms}
	\end{table}
...
\begin{figure*}[t]
	...
	\begin{tikzpicture}[
		...
		box/.style={rectangle, draw, minimum width=2.2cm, minimum height=1cm, align=center, font=\footnotesize},
		smallbox/.style={rectangle, draw, minimum width=1.2cm, minimum height=0.8cm, align=center, font=\scriptsize},
		arrow/.style={->, >=stealth, thick}
		]
		
		% Title note
		\node[above, font=\small, text width=10cm, align=center] at (0,4.2) {\textit{Sample BHC Hierarchy with Reports required for each entity}};
		
		% Top level - BHC
		\node[box] (bhc) at (0,3) {\textbf{BHC}\\FR Y-9C\\FR Y-9LP\\FR Y-15};
		
		% Second level
		\node[box] (bank1) at (-4.5,1) {\textbf{Bank 1}\\FFIEC 041};
		\node[box] (bank2) at (-1.5,1) {\textbf{Bank 2}\\FFIEC 031};
		\node[box] (nonbank1) at (1.5,1) {\textbf{NonBank 1}\\FR Y-11};
		\node[box] (nonbank2) at (4.5,1) {\textbf{NonBank 2}\\FR 2314};
		
		 % Third level - Bank 1 subsidiaries
		\node[smallbox] (ussub1) at (-5.5,-1.5) {\textbf{US Sub 1}\\FR Y-11};
		\node[smallbox] (ussub2) at (-3.7,-1.5) {\textbf{US Sub 2}\\FR Y-11};
		
		% Third level - Bank 2 subsidiaries
		\node[smallbox] (ussub3) at (-2.1,-1.5) {\textbf{US Sub 3}\\FR Y-11};
		\node[smallbox] (nonussub) at (-0.3,-1.5) {\textbf{Non US Sub}\\FR 2314};
		
		% Arrows from BHC
		\draw[arrow] (bhc) -- (bank1);
		\draw[arrow] (bhc) -- (bank2);
		\draw[arrow] (bhc) -- (nonbank1);
		\draw[arrow] (bhc) -- (nonbank2);
		
		% Arrows from Bank 1
		\draw[arrow] (bank1) -- (ussub1);
		\draw[arrow] (bank1) -- (ussub2);
		
		% Arrows from Bank 2
		\draw[arrow] (bank2) -- (ussub3);
		\draw[arrow] (bank2) -- (nonussub);
		
	\end{tikzpicture}
	\caption{Bank Holding Company Organizational Hierarchy with Required Regulatory Reports}
	\label{fig:bhc_hierarchy}
\end{figure*}		
	...
	\subsection{FR Y-9LP Parent only financial statements for BHCs}
		...
		\subsection{FR Y-15 Systemic Risk Report}
		  ...
		  \subsection{FFIEC-002 - Report of Assets and Liabilities of U.S. Branches and Agencies of Foreign Banks}
		  
		  Foreign banks with operations in the US can be classified as those which have operations which are significant enough to form an Intermediate Holding Company(IHC) for their US operations or can continue to have a foreign branch alone. The mandate of an IHC comes when the foreign bank has more than \$ 100 billion of assets in US. If branches of a foreign bank have less than \$ 100 billion of assets they need not form an IHC but will still have register as a national bank or under one of the 50 state charters. 
		  A typical structure of a Foreign Banking Office(FBO) of an international bank is given in fig \eqref{fig:ihc_hierarchy}.
...
\begin{figure*}[t]
	...
	\begin{tikzpicture}[
		...
		box/.style={rectangle, draw, minimum width=2.2cm, minimum height=1cm, align=center, font=\footnotesize},
		smallbox/.style={rectangle, draw, minimum width=1.8cm, minimum height=0.8cm, align=center, font=\scriptsize},
		topbox/.style={rectangle, draw, minimum width=2.5cm, minimum height=1cm, align=center, font=\footnotesize},
		subgraph/.style={rectangle, draw, dashed, thick, minimum width=8cm, minimum height=2.5cm, fill=gray!10},
		arrow/.style={->, >=stealth, thick}
		]
		
		% Top level - Foreign Parent
		\node[topbox] (parent) at (0,5) {\textbf{Foreign Parent}};
		
		% Subgraph box for CUSO
		\node[subgraph] (cuso) at (-0.5,3) {};
		\node[above right, align=left, font=\small\bfseries] at (cuso.south west) {CUSO\\FR Y-15};
		
		% First level - FBO Branch and IHC (inside subgraph)
		\node[box] (fbo) at (-3,3) {\textbf{US Branch of Parent}\\FFIEC-002};
		\node[box] (ihc) at (2,3) {\textbf{IHC}\\FR Y-9C\\FR Y-9LP\\FR Y-15};
		
		% Second level - IHC subsidiaries
		\node[box] (bank1) at (-2,0.8) {\textbf{Bank without}\\{\textbf{foreign subs}}\\FFIEC-041};
		\node[box] (bank2) at (0.5,0.8) {\textbf{Bank with}\\{\textbf{foreign subs}}\\FFIEC-031};
		\node[box] (nonbank1) at (3.2,0.8) {\textbf{US Nonbank}\\FR Y-11};
		\node[box] (nonbank2) at (5.5,0.8) {\textbf{Non-US Nonbank}\\FR 2314};
		
		% Third level - Bank subsidiaries
		\node[smallbox] (ussub1) at (-3,-1.5) {\textbf{US Nonbank}\\FR Y-11};
		\node[smallbox] (ussub2) at (-1.0,-1.5) {\textbf{US Nonbank}\\FR Y-11};
		\node[smallbox] (ussub3) at (1.0,-1.5) {\textbf{US Nonbank}\\FR Y-11};
		\node[smallbox] (nonussub) at (2.9,-1.5) {\textbf{Non-US}\\{\textbf{Nonbank}}\\FR 2314};
		
		% Arrows from Parent
		\draw[arrow] (parent) -- (fbo);
		\draw[arrow] (parent) -- (ihc);
		
		% Arrows from IHC to second level
		\draw[arrow] (ihc) -- (bank1);
		\draw[arrow] (ihc) -- (bank2);
		\draw[arrow] (ihc) -- (nonbank1);
		\draw[arrow] (ihc) -- (nonbank2);
		
		% Arrows from Bank 1 to subsidiaries
		\draw[arrow] (bank1) -- (ussub1);
		\draw[arrow] (bank1) -- (ussub2);
		
		% Arrows from Bank 2 to subsidiaries
		\draw[arrow] (bank2) -- (ussub3);
		\draw[arrow] (bank2) -- (nonussub);
		
		% Labels for third level entities
		\node[below=0.1cm of ussub1, font=\tiny] {USSub 1};
		\node[below=0.1cm of ussub2, font=\tiny] {USSub 2};
		\node[below=0.1cm of ussub3, font=\tiny] {USSub 3};
		\node[below=0.1cm of nonussub, font=\tiny] {NonUS Sub};
		
	\end{tikzpicture}
	\caption{IHC (Intermediate Holding Company) Organizational Hierarchy with Required Regulatory Reports}
	\label{fig:ihc_hierarchy}
\end{figure*}
		  
		  These banks will also have to file the FFIEC-002 report regardless of the asset size. The requirement to file FFIEC-002 comes from the International Banking act of 1978\cite{FFIEC002}.
		  
		  The FFIEC-002 report is a cross between the FR Y-9C and FFIEC-031 reports in regard to the schedules in the report with the following differences
		  
		  \begin{enumerate}
		  	\item The FFIEC-002 report has only a schedule for the Report of Assets and Liabilities and cash flow. However the income statement is not part of FFIEC-002
		  	\item FFIEC-002 does not have a Capital and RWA schedule.
		  	\item FFIEC-002 also does not contain a Trading Assets and Liabilities schedule
		  	\item FFIEC-002 does not contain the schedule for Variable Interest entities because the concept of VIE is unique to US-GAAP
		  	\item While the data of FFIEC-002 is public, only Schedule M - Amounts due from and due to related institutions is confidential
		  	\item Schedule P has been repurposed to act as the schedule for other borrowed money. FR Y-9C , FFIEC-031 and FFIEC-041 have their HC-P and RC-P for 1-4 Residential Mortgage banking activities.
		  	\item Similar to Call reports the schedules of Sch O for Data relating to Deposit insurance and Sch T for Fiduciary activities need to be reported in FFIEC-002 which is not the case with FR Y-9C
		  \end{enumerate}
		  
		  The FFIEC-002 report will have to be filed by foreign bank branches on a quarterly basis at the end of the quarter. Because there is no minimum threshold for filing , the FFIEC-002 report can serve as a rich data source for looking at the activities of foreign banks in the US.
		  
		  \subsection{FR-2314- Financial Statements of Foreign Subsidiaries of U.S. Banking Organizations}
		  
		  FR-2314 is a report which details the Financial statements of the foreign subsidiary of a US Bank. So the FR-2314 acts as the FR Y-11 for the foreign subsidiaries of a US Bank. FR-2314 started as FR 314 in the 1960s and subsequently made as FR-2314 in 1978 \cite{FR2314}. There have been modifications made to this report in between  1978 and now but the report purpose has more or less remained the same. FR-2314 is to be filed on a quarterly basis for nonbank subsidiaries with parents having assets over \$ 500 million with the subsidiary having either of the below conditions
		  
		  \begin{itemize}
		  	\item Total assets of over \$1 Billion
		  	\item Off balance sheet transactions of over \$5 billion
		  	\item Equity capital of more than 5\% of the Parent entity
		  	\item Operating revenue of more than 5\% of the Parent entity
		  \end{itemize}
		  
		  In case these requirements are not met and the total assets of the foreign nonbank subsidiary is between \$ 250 million and \$ 500 million the FR-2314S is to be filed annually.
		  
		  With regard to the schedules of FR-2314, the schedules are the same as the FR Y-11 except for an additional schedule of BD-Q where the subsidiary has to the report amounts due to and due from the subsidiary.
		  
		  \subsection{Detail Loan Level Reports}
		  	The FR Y-14Q and FR Y-14M reports are a set of reports which go into the detail level of data which is provided in an aggregated form in the FR Y-9C. The FR Y-14Q report refers to a set of schedules which publish the detail level on FR Y-9C on a quarterly basis and FR Y-14M concentrates on home mortgages and Home equity lines of credit and is to be reported on a monthly basis. Each of FR 14-Q and FR Y-14M have got a set of schedules which are independent of each other and so actually act more as reports themselves. We will be looking at these reports in this section in detail and how they tie back to loan analytics.
		  
		  \subsubsection{FR Y-14Q - Capital Assessments and Stress Testing}
		       The FR Y-14Q is a detailed level view of the data of FR Y-9C some of which are used by the Fed for the Comprehensive Capital Analysis and Review(CCAR) analyses done for various banks\cite{FRY14Q}. The threshold for assets for filing FR Y-14Q is \$ 100 billion. It has been disaggregated into various schedules , each of which are independent of each other with data which are distinct from each other. Because of the distinct schema of the data reported in each schedule of FR Y-14Q, there cannot be one integrated submission of FR Y-14Q unlike the other reports which have been discussed till now. This is because the level of granularity of each schedule of FR Y-14Q is different. Table \ref{tab:FRY14Q_Schedules} provides the nature of each FR Y-14Q schedule.
		       
		       \begin{table}[htbp]
		       	\caption{FR Y-14Q Schedules and Nature}
		       	\label{tab:FRY14Q_Schedules}
		       	\centering
		       	\begin{tabular}{|l|p{4cm}|}
		       		\hline
		       		\textbf{Schedule Name} & \textbf{Nature of the Schedule} \\
		       		\hline
		       		Schedule A & Retail Schedule \\
		       		\hline
		       		Schedule B & Securities Schedule \\
		       		\hline
		       		Schedule C & Regulatory Capital Instruments Schedule \\
		       		\hline
		       		Schedule D & Regulatory Capital Transactions Schedule \\
		       		\hline
		       		Schedule E & Operational Risk Schedule \\
		       		\hline
		       		Schedule F & Trading Schedule \\
		       		\hline
		       		Schedule G & Pre-Provision Net Revenue (PPNR) Schedule \\
		       		\hline
		       		Schedule H & Wholesale Loan Schedule \\
		       		\hline
		       		Schedule J & Retail Fair Value Option/Held for Sale Schedule \\
		       		\hline
		       		Schedule K & Balances Schedule \\
		       		\hline
		       		Schedule L & Counterparty Schedule \\
		       		\hline
		       		Schedule M & Supplemental Schedule \\
		       		\hline
		       	\end{tabular}
		       \end{table}
		       
		   Out of the above schedules for Loan data analysis , there is a lot of information in Schedule H - Wholesale Loans and Schedule J - Retail FVO/HFS . 
		   
		   Schedule H deals with Wholesale loans and is divided into four Sub-schedules
		   
		   \begin{enumerate}
		   	\item Corporate (H1)
		   	\item Commercial Real Estate (H2)
		   	\item Line of Business(H3)
		   	\item Internal Risk Rating (H4)
		   \end{enumerate}
		   
		   Each sub schedule contains detailed information about the data point. For example H1 will contain detailed data about Corporate Loans.
		 
		  Schedule J is an aggregated schedule similar to FR Y-9C. It contains aggregated numbers about the bank's retail loans which are accounted under Fair Value Option(FVO) . Loans under FVO are usually Held for Sale(HFS) or Held for Investment(HFI). Schedule J has got two tables. Table 1 is about the composition of the retail loan book of the bank in terms of the loan products. Table 2 is about the vintage of the loan and the lease.
		  
		  Because of the disaggregated nature of data in some schedules of FR Y-14Q, it is considered confidential and the Fed publishes only summary information from FR 14-Q as part of the Dodd Frank Annual Stress Test (DFAST) results.
		   
		\subsubsection{FR Y-14M - Monthly Capital Assessments and Stress Testing} 
		    The FR Y-14M is a monthly report containing detailed loan level and portfolio level data. Like the FR Y-14Q it is collected as part of the CCAR process. The FR Y-14M consists of 4 schedules. Three of the schedules will have to be reported at a portfolio and a loan level. There is also a Detailed address matching schedule. Table \eqref{tab:FRY14M_Schedules}provides the details of each of the FR Y-14M schedules. 
		    
		 \begin{table}[htbp]
		 	\caption{FR Y-14M Schedules and Nature of each schedule}
		 	\label{tab:FRY14M_Schedules}
		 	\centering
		 	\begin{tabular}{|p{1cm}|p{3.5cm}|p{1.5cm}|}
		 		\hline
		 		\textbf{Schedule Name} & \textbf{Description} & \textbf{Level of Reporting} \\
		 		\hline
		 		Schedule A & Domestic First Lien Closed End 1-4 Family Residential Loan & Portfolio and Loan Level \\
		 		\hline
		 		Schedule B & Domestic Home Equity Loan and Home Equity Line & Portfolio and Loan Level \\
		 		\hline
		 		Schedule C & Address matching & Detail Address level \\
		 		\hline
		 		Schedule D & Domestic Credit Card Data Collections & Portfolio and Loan Level \\
		 		\hline
		 	\end{tabular}
		 \end{table}
		    
		    The FR Y-14M like the FR Y-14Q is summarized and reported as part of the DFAST results to the public with the detailed data being confidential.
		    
	\subsection{Selection of Report Data for Analysis}
	 
	 Figure \eqref{fig:bhc_hierarchy} provides a sample hierarchy for BHCs and the reports that each legal entity will have to file. The BHC will have to file FR Y-9C, FR Y-9LP and FR Y-15 while the subordinate entities will have file FFIEC-031/FFIEC-041 if they are banks and FR Y-11/FR-2314 if they are non-banks.
	 
	 The selection of data for the use case may depend upon various factors like the type of analysis to be done, the level of detail required, the available time and the amount of resources required. 
	 
	 If only BHCs are to be analyzed at a consolidated bank level with loan data being sufficient at an aggregated basis, then the FR Y-9C would be the best choice. However the FR Y-9C suffers from the limitation that only aggregate data is provided and for Intermediate Holding companies(IHCs) of foreign banks, do not provide sufficient information about the Combined US Operations(CUSO) of these foreign banks. The CUSO operations of foreign banks comprise of the IHC plus the local branch of the foreign bank.
	 
	 If the local branch of the foreign bank is to be analyzed on a standalone basis, the FFIEC-002 can be considered. The FFIEC-002 has no threshold , so any foreign bank having a branch in USA is required to file. The data is also available in public. So this is a good resource in case foreign bank branch loan activity is to be analyzed.
	 
	 The FR Y-9LP is a report at the head office level and so is useful for head office activities like inter-bank lending. The FR Y-15 can be used to analyze the systemic risk posed by a single bank to the US Banking environment.
	 
	 Call reports like FFIEC-031 , FFIEC-041 and FFIEC-051 can be used for loan analysis at a deposit taking bank level at an aggregate basis. The dataset provided in these reports are very similar to FR Y-9C and are readily available making these reports a valuable resource for the loan activities of deposit taking bank entities.
	 
	 For non-bank subsidiaries, the FR Y-11 and FR-2314 data while not very detailed, provide sufficient granularity to lend themselves for loan analysis. However the FR Y-11 and FR-2314 report data are not available without a FOIA request which does not augur well for data availability.
	 
	 At a detailed loan level, the NCUA reports provide detailed loan level detail for Credit Unions and are available for public consumption. For lending details of large banks, the FR Y-14Q Schedule H reports for Wholesale Loans and the FR Y-14M reports are good resources to have, but given the level of detail, the Fed would require a good justification for release of data for research.
	
	\section {Analytical themes for Loan Portfolio research}
	
	The previous section introduced the rich datasets available with the public data which can extracted from the Federal Reserve and FFIEC websites and how the datasets can be used for Bank Data Analysis. In this section we will look at the themes that can be researched in detail based on the available dataset especially involving the FR Y-9C. This section will also try to see if there are any publications on loan data analysis on the theme and provide a brief description of the publication. It is to be noted that the analytical themes provided in this section is by no means exhaustive and complete and the total amount of analytical themes at disposal with the FR Y-9C dataset is something that cannot be boxed into a section.
	
	\subsection{Discussion of Analytical themes available}
		Though there are endless data analyses directions possible using the data FR Y-9C there are some themes listed below. Each of the subsections listed below will have a basic level analysis done with the anonymized data of a bank taken from the publicly available FR Y-9C data.
		\subsection{Product analysis}\label{product_decomp}
		 Schedule HC-C of FR Y-9C has Loan details which are HFI and HFS which are segregated by Collateral , Borrower, Purpose or a combination of these. We can use this data in HC-C to find the product mix in the loan portfolio of a bank which can in-turn be used for analysis. The product mix of a bank can be used in combination of sensitivity analysis to find out the sensitivity of a bank to a risk factor like interest rate. All of this can be analyzed by looking at the aggregated data presented in public report of the bank like the FR Y-9C report.
		 
		 A simple analysis is carried out here on the product mix of a BHC's FR Y-9C using the numbers reported in HC-C alone.
		 
		 \begin{table}[htbp]
		 	\centering
		 	\caption{HFI HFS Loan Distribution Summary}
		 	\label{tab:loan_distribution}
		 	\begin{tabular}{|p{0.8cm}|p{2.2cm}|p{1.2cm}|p{1.5cm}|p{1.3cm}|}
		 		\hline
		 		\textbf{Sl. No} & \textbf{Description} & \textbf{MDRM} & \textbf{Amount in 1000 \$} & \textbf{Percent of} \\
		 		\hline
		 		1 & Loans secured by real estate(Consolidated Level) & BHCK 1410 & 56992863 & 58\% \\
		 		\hline
		 		4.a & Commercial loans to US addresses(Consolidated Level) & BHCK 1763 & 3329788 & 24\% \\
		 		\hline
		 		4.b & Commercial loans to non US addresses(Consolidated Level)& BHCK 1764 & 66632 & 0\% \\
		 		\hline
		 		6.c & Automobile loans(Consolidated Level) & BHCK K137 & 3764610 & 4\% \\
		 		\hline
		 		12 & Total Loans (Consolidated Level) & BHCK 2122 & 97757548 & 100\% \\
		 		\hline
		 	\end{tabular}
		 \end{table}
		 
		 As we can see from the above the bank has 60 \% of its loan portfolio in real estate loans with small portions in commercial and industrial and automobile loans.
		 
		 \subsection{Concentration}\label{conc_ratios}
		 Concentration is the percent of assets that exists with a specific counterparty, asset or asset class. IMF has two categories of concentration \cite{ConcRiskIMF}, by counterparty or by sector. Concentration is part of the Pillar II in Basel II and Basel III rules. Because Pillar II gives a lot of discretion for measurement and quantification, concentration risk is measured by individual regulators at each jurisdiction. In the US, the Office of the Comptroller of currency(OCC) defines concentration as the amount of exposure to a single counterparty or a group of related counterparties\cite{OCCConc}. Because a group of related counterparties form a sector , concentrations can be measured by the type of counterparty that a bank has exposure to. The threshold for concentration has been by the OCC as 25\% , so anything above 25\% is considered concentrated.
		 
		 In our example in table \eqref{tab:concentration_table} , while mortgages form only 15.8\% of the total loan book which is below the 25\% threshold for concentration set by the OCC, home mortgages make up 27\% of overall loans secured by real estate.
		  
		 \begin{table}[htbp]
		 	\centering
		 	\caption{Loan Distribution by Category}
		 	\label{tab:concentration_table}
		 	\scriptsize
		 	\begin{tabular}{|p{0.6cm}|p{1.8cm}|p{0.9cm}|p{1.1cm}|p{0.9cm}|p{0.7cm}|}
		 		\hline
		 		\textbf{Sl. No} & \textbf{Description} & \textbf{MDRM} & \textbf{Amount in 1000 \$} & \textbf{Percent of total HFI/HFS Loan book} & \textbf{Percent} \\
		 		\hline
		 		1 & Loans secured by real estate & BHCK 1410 & 56992863 & 58.30\% & 100.00\% \\
		 		\hline
		 		1.c.1 & Loans secured by 1-4 family residential properties - Revolving Open End Loans - Domestic Offices & BHCK 1797 & 4203358 & 4.30\% & 7.38\% \\
		 		\hline
		 		1.c.2.a & Loans secured by 1-4 family residential properties - Closed End Loans First Lien - Domestic Offices & BHCK 5367 & 15450206 & 15.80\% & 27.11\% \\
		 		\hline
		 		1.c.2.a & Loans secured by 1-4 family residential properties - Closed End Loans Junior Lien - Domestic Offices & BHCK 5368 & 80500 & 0.08\% & 0.14\% \\
		 		\hline
		 		1.d & Loans secured by multifamily properties - Domestic Offices & BHCK 1460 & 4610954 & 4.72\% & 8.09\% \\
		 		\hline
		 		12 & Total Loans & BHCK 2122 & 97757548 & 100.00\% & Not applic \\
		 		\hline
		 	\end{tabular}
		 \end{table}
		 
		 \subsection{Loan Loss Reserves}\eqref{loss_ratios}
		 Another indicator that can be checked and analyzed by aggregate reports is the amount of loan loss reserves in the bank and the change in the reserves each quarter. Schedule HI-B contains data on the changes to the loss reserves and HI-C contains detailed data on the loss reserves held at bank. The seasonality factor can also be measured by a report like the FR Y-9C. The loan loss reserves can also be analyzed based the economic cycle that the economy is and the allowances maintained by the bank. 
		 
		 Table \eqref{tab:loan_allowance} provides the allowance percentage for each category of real estate loans. At 1.04\% residential real estate loans have the lowest allowance amongst real estate loans. Column 1 provides the amortized cost which is the annual payment - interest payments.
		 
		 \begin{table}[htbp]
		 	\centering
		 	\caption{Loan Categories with Amortized Cost and Allowance}
		 	\label{tab:loan_allowance}
		 	\scriptsize
		 	\begin{tabular}{|p{0.4cm}|p{1.2cm}|p{0.7cm}|p{0.9cm}|p{0.7cm}|p{0.9cm}|p{0.6cm}|}
		 		\hline
		 		\textbf{Sl No} & \textbf{Line Item} & \textbf{MDRM} & \textbf{Amortized Cost Amount} & \textbf{MDRM2} & \textbf{Allowance Amount} & \textbf{\% Allowance} \\
		 		\hline
		 		1.a & Construction Loans & BHCK J04 & 9571187 & BHCK J12 & 232917 & 2.43\% \\
		 		\hline
		 		1.b & Commercial Real Estate Loans & BHCK J05 & 27332822 & BHCK J13 & 343404 & 1.26\% \\
		 		\hline
		 		1.c & Residential Real Estate Loans & BHCK J06 & 19390634 & BHCK J14 & 202590 & 1.04\% \\
		 		\hline
		 	\end{tabular}
		 \end{table}
		 \subsection{Asset Quality}\label{acc_ratios}
		 As has been mentioned in the previous sections, HC-N has got a the delinquency rates for a same MDRM in HC-C for almost all the MDRMs in HC-C in FR Y-9C. This makes a one on one comparison of asset quality very easy to make using the FR Y-9C. We can also throw in the product vise asset quality checks that can be done using the FR Y-9C . All of these provide a huge window of opportunities for data analysis using FR Y-9C data.
		 
		 Table \eqref{tab:loan_past_due} provides the amounts in each category of loans among residential and commercial loans . Also the percentage of loans at non-accrual is computed in the last column. The rich dataset provides a lot of opportunities to analyze the loan books of various banks operating in the US as can be visualized in the table.
		 
		 
		 \begin{table}[htbp]
		 	\centering
		 	\caption{Loan Past Due and Nonaccrual Status (figures in 1000\$)}
		 	\label{tab:loan_past_due}
		 	\scriptsize
		 	\begin{tabular}{|p{1.2cm}|p{0.9cm}|p{0.8cm}|p{0.8cm}|p{0.8cm}|p{0.9cm}|}
		 		\hline
		 		\textbf{Description} & \textbf{HC-C Value} & \textbf{HC-N Col A - Past due till 89 days} & \textbf{HC-N Col B - Past due $>$90 days} & \textbf{HC-N Col C - Non accrual} & \textbf{Percentage of Non accrual loans} \\
		 		\hline
		 		Loans secured by 1-4 family residential properties - Revolving Open End Loans - Domestic Offices &  4203358 & 32929 & 0 & 71445 & 1.70\% \\
		 		\hline
		 		Loans secured by 1-4 family residential properties - Closed End Loans First Lien - Domestic Offices &  15450206 & 243032 & 478990 & 420428 & 2.72\% \\
		 		\hline
		 		Loans secured by 1-4 family residential properties - Closed End Loans Junior Lien - Domestic Offices &  80500 & 1289 & 36 & 7944 & 9.87\% \\
		 		\hline
		 		Commercial and Industrial Loans to US Addresses & 56992863 & 45030 & 10703 & 243017 & 0.43\% \\
		 		\hline
		 	\end{tabular}
		 \end{table}
		 
		 \subsection{Systemic Risk}\label{sys_decomp}
		 Systemic risk is directly not connected with Loan analysis, but the six systemic risk factors that drive FR Y-15 namely Size, Interconnectedness, Substitutability, Complexity, Cross Jurisdictional activity and short term wholesale funding will indirectly impact factors like loan availability, interest rates, delinquency rates and reserves.
		 
		 The FR Y-9C does not have the aggregated data that the Fed uses to determine systemic risk in a single MDRM. However some of the details required for measuring systemic risk can be inferred from FR Y-9C.
		 The variables used by the Fed for assessing systemic risk has been detailed in the report FR Y-15. A sample piece of MDRM from FR Y-15 and how the data can be provided from the FR Y-9C as well is discussed in this subsection.
		 
		 For our analysis, we will be taking the Line Gross Notional amount of derivative items subject to 20\% CCF. This is represented by the MDRM RISKM718 for US Headquartered banks and RISIM718 for FBOs of Foreign Banks which are required to file FR Y-15. 
		 
		 The same number can be derived from the schedule HC-R Part II of FR Y-9C as has been shown in \eqref{tab:off_balance_sheet}.
		 
		 \begin{table}[htbp]
		 	\centering
		 	\caption{Off-Balance Sheet Items by Report Type}
		 	\label{tab:off_balance_sheet}
		 	\scriptsize
		 	\begin{tabular}{|p{0.6cm}|p{3.2cm}|p{1.2cm}|p{0.8cm}|p{1.2cm}|}
		 		\hline
		 		\textbf{Sl. No} & \textbf{Line Item} & \textbf{MDRM} & \textbf{Report} & \textbf{Amount} \\
		 		\hline
		 		4.b & Gross Notional amounts for items with 20\% CCF & RISKM718 & FR Y-15 & 2984843 \\
		 		\hline
		 		18.a & Unused commitments - maturity of one year or less - 20\% CCF & BHCKS525 & FR Y-9C & 2980947 \\
		 		\hline
		 		14 & Commercial and similar letters of credit - 20\% CCF & BHCKG806 & FR Y-9C & 3896 \\
		 		\hline
		 		\multicolumn{2}{|l|}{\textbf{Total FR Y-9C : 2984843}} & & \multicolumn{2}{r|}{\textbf{ Total FR Y-15:2984843}} \\
		 		\hline
		 	\end{tabular}
		 \end{table}
		 
		 In a similar way a lot of Systemic risk parameters provided in FR Y-15 can be derived from FR Y-9C. And the detailed nature of MDRMs in FR Y-9C provides a more granular number which can be sliced and diced in an easier fashion.
		 
		 \subsection{Liquidity}\label{liq_trend}
		 While not directly connected with Loan data analysis, the liquidity factor plays out in terms of the market for various debt based instruments like corporate bonds , mortgage backed securities, asset backed securities and government bonds. The liquidity factor also plays out in the derivative market as well in determining the spreads for all instruments in the derivative market. 
		 
		 Liquidity data for banks can also be analyzed using the data available in Schedules HI and HC of FR Y-9C. It needs to be mentioned that if a liquidity measure like cash flow is to be monitored at an aggregate basis, the FR Y-9LP would have data which is fit for purpose. With regard to the daily/monthly liquidity positions the FR-2052a would be the right report.
		 
		 The FR Y-9C is most suitable to see the aggregate positions in liquidity inducing trades like trades in repos. In table \eqref{tab:securities_financing}, we can see that the liquidity infused by repo transactions has increased by 156\% quarter over quarter.
		 
		 \begin{table}[htbp]
		 	\centering
		 	\caption{Securities Financing Transactions}
		 	\label{tab:securities_financing}
		 	\scriptsize
		 	\begin{tabular}{|p{0.5cm}|p{2cm}|p{0.9cm}|p{0.7cm}|p{0.9cm}|p{0.7cm}|p{0.5cm}|}
		 		\hline
		 		\textbf{Sl. No} & \textbf{Line description} & \textbf{MDRM} & \textbf{Report} & \textbf{Current Quarter Amount} & \textbf{Previous quarter amount} & \textbf{Diff. in \%} \\
		 		\hline
		 		3b & Securities purchased under agreements to resell & BHCK B989 & FR Y-9C & 0 & 0 & \\
		 		\hline
		 		14.b & Securities sold under agreements to repurchase & BHCK B995 & FR Y-9C & 152515 & 594082 & 156\% \\
		 		\hline
		 	\end{tabular}
		 \end{table}
		 
	\section{Technique selection}
	   The analytical themes listed above can be taken as a good start for loan dataset analysis provided by the FR Y-9C. This section will provide some of the techniques at a researcher's disposal for exploring the themes provided.
	   
	   \subsection{Descriptive Statistics}
	       The small examples given in the previous section were all examples of Analysis of the Descriptive Statistics provided from the data in FR Y-9C. At its core , descriptive statistics enables to understand the nature of the underlying data and provide the required details to summarize and describe the nature of the underlying data. The analysis is usually deterministic and no probability tests are carried out unlike inferential statistics.
	       
	       Description statistics produced are usually in the form of ratios , growth rates,trends and decomposition. In our case Sections \eqref{conc_ratios},\eqref{loss_ratios},\eqref{acc_ratios} use ratios to determine a result while sections on \eqref{product_decomp} and \eqref{sys_decomp} talk about decomposition of the loan book. The section on Liquidity \eqref{liq_trend} is on the trend data.
...
\bibitem{FR2314} “Federal Reserve Board - Reporting Forms - FR2314.” Accessed: Oct. 22, 2025. [Online]. Available: \url{https://www.federalreserve.gov/apps/reportingforms/Report/Index/FR_2314FR_2314S}
\bibitem{FRY14Q} "Federal Reserve Board - Reporting Forms - FR Y-14Q", Accessed:Oct 27,2025 [Online]. Available \url{https://www.federalreserve.gov/apps/reportingforms/Report/Index/FR_Y-14Q}
\bibitem{ConcRiskIMF} "Measuring Concentration Risk A Partial Portfolio
Approach", Accessed:Nov 8,2025 [Online]. Available: \url{https://www.imf.org/external/pubs/ft/wp/2016/wp16158.pdf}
\bibitem{OCCConc} "Concentrations of Credit" Accessed : Nov 8,2025 [Online]. Available: \url{https://www.occ.gov/publications-and-resources/publications/comptrollers-handbook/files/concentrations-of-credit/pub-ch-concentrations.pdf} 
